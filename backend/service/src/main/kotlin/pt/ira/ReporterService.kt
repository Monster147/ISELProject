...
class ReportService(
    ...
    fun findById(id: Int): Either<ReportError, Report> {
        return trxManager.run {
            val report = repoReport.findById(id)
    ...
    fun findByIntervenor(intervenor: Intervenor): List<Report> =
        trxManager.run { repoReport.findByIntervenor(intervenor) }

    fun findAll(): List<Report> =
        trxManager.run { repoReport.findAll() }

    fun addEditor(reportId: Int, userId: Int): Either<ReportError, Report> {
        return trxManager.run {
            val report = repoReport.findById(reportId)
                ?: return@run failure(ReportError.ReportNotFound)
    ...
    fun removeEditor(reportId: Int, userId: Int): Either<ReportError, Report> {
        return trxManager.run {
            val report = repoReport.findById(reportId)
                ?: return@run failure(ReportError.ReportNotFound)
    ...
    fun updateStatus(reportId: Int, status: ReportStatus): Either<ReportError, Report> {
        return trxManager.run {
            val report = repoReport.findById(reportId)
                ?: return@run failure(ReportError.ReportNotFound)
    ...
    fun addIntervenor(reportId: Int, intervenorId: Int): Either<ReportError, Report> {
        return trxManager.run {
            val report = repoReport.findById(reportId)
                ?: return@run failure(ReportError.ReportNotFound)

            val intervenor = repoIntervenor.findById(intervenorId)
    ...
    fun removeIntervenor(reportId: Int, intervenorId: Int): Either<ReportError, Report> {
        return trxManager.run {
            val report = repoReport.findById(reportId)
                ?: return@run failure(ReportError.ReportNotFound)
